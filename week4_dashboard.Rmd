...
defn_term <- function(term, text) {
  term_node <- if (is.character(term)) tags$strong(term) else term
...
tags$div(
  ...
  selectInput(inputId = "method", label = "Preprocessing Method", 
              choices = c("Raw" = "raw",
                          "Random Forest (Abate et al., 2019)" = "rf",
                          "XGBoost" = "xgb"), selected = "xgb"),
  tags$div(
  ...
  tags$div(
    ...
    conditionalPanel(condition = "input.method == 'xgb'",
      checkboxGroupInput(
        inputId = "mods", label = NULL,
        choiceNames = list(
    ...
    checkboxGroupInput(
      ...
      choiceNames = list(
        defn_term("Viewsheds",
        "Viewsheds are visibility rasters computed from the DTM for a selected observer location. Black cells indicate terrain predicted to be visible.")),
      choiceValues = c("vis"),
      selected = character(0)
    ),
    ...
    conditionalPanel(
      ...
      tags$div(
        ...
        defn_term(
          tags$em("What is this?"),
        "Viewpoints are predefined locations on the site used as the observer position for the viewshed calculation."),
        selectizeInput(
          inputId  = "vp",
          label = NULL,
          choices = c(
